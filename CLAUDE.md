...
- **In progress:** Naturalness steps 1-4 are built. Step 5 is *use it* — drill particles for a fortnight before any more content gets written, because the findings will say where the gap actually is. Then a second category (reactions) to prove the contract is genuinely category-agnostic, then the ride-along detector in the other modes, then voice.
...
| **Naturalness** | Drills what is grammatical but not native. Particles built out; six more categories declared but empty |
...
- `src/naturalness/` — the Naturalness Layer. `categories.js` (the seven categories), `contract.js` (the language-module contract, JSDoc not TypeScript — see NATURALNESS.md), `scoring.js` (reach + hit rate), `drillEngine.js` (scheduling, presentation, the judge request), `modules/de.js` (German content). UI rendering stays in `App.jsx`; only pure logic and content live here
