...
- **Restructured 2026-09-13 after first real use:** the layer originally shipped as ONE "Naturalness" mode with the seven areas as chips inside it. Daniel opened the app, saw only the ride-along toggle, and could not find the drills at all — the areas were a level down behind a truncated dropdown label. **Each area is now its own mode**, picked in the same dropdown as Grammar. If anything like this recurs, the lesson is that a new capability belongs where the user already looks, not behind a new container. Also added a **server-render test** (`renderToString` in Node via esbuild) after a temporal-dead-zone `ReferenceError` nearly shipped — a dependency array referencing a `const` declared further down white-screens the app and esbuild cannot see it.
- **Drill wording fixed 2026-09-13 after real use:** two reports, one cause — the prompt did not say what to do with what was on screen. A `soften` item read *"Say the same thing, but pitched as described"* above a bare `[ correcting without accusing ]`, and nothing connected them; a `minimalPair` showed two dialogue lines both starting `A:`, so both looked labelled A. **Every extra line is now labelled** for its format (Pitch it as / Attitude / Situation / Context / Options / Note), minimal pairs are **numbered 1 and 2**, and every instruction states the answer language. A test asserts that last point across all 245 items — it caught `which` saying neither. If a new format is added, label its extras and say the language.
- **Teaching on a miss, added 2026-09-13:** being corrected without being taught is useless — if a drill wanted `wohl` and it never came, the word itself needs explaining. A miss (or a skip) now appends **what the target means plus a further worked example**. The module's own `reference` gloss is preferred over anything generated, because it was written deliberately and carries its caveats; `exampleFor()` reuses a *different* drill item's English-to-German pair so the example is curated too. **Guard:** a curated example is used ONLY where a curated gloss exists for the same feature — categories that tag every item with a generic name (`collocation`) would otherwise match an unrelated item and present it as "another example" of the thing just missed. Everything else falls back to the judge, which now returns `meaning` and `example` (empty when the answer landed, so a correct answer is not lectured at — measured 109 output tokens on a pass vs 206 on a miss).
...
| **Naturalness** (7 modes) | Each area is its OWN mode in the dropdown, under a "Naturalness" optgroup: Modal particles, Reactions, Turn structure, Register & softening, Self-rescue, Collocations, English traps — plus "Mixed — all areas". 245 items. Level and Topic are hidden in these modes because they do not apply |
